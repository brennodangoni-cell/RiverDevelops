...
export default function VideoLab() {
    ...
    const handleRegenerateWithFeedback = async (index: number) => {
        ...
        if (!feedback?.trim()) {
            toast.error('Escreva o que voc├¬ n├úo gostou para melhorar!');
            return;
        }
        ...
        toast.promise(
            ...
            })().finally(() => setLoadingIndices(prev => prev.filter(i => i !== index))),
            {
                ...
                success: 'Nova vers├úo gerada com sucesso!',
    ...
    const handleRenderAllVisible = async () => {
        ...
        if (missingIndices.length === 0) {
            toast.success('Todos os mockups j├í foram renderizados!');
            return;
        }
    ...
    const saveToFavorites = () => {
        ...
        toast.success('Ô¡É Projeto salvo nos favoritos!');
    };
                                ...
                                <div className={`${previewUrls.length > 0 ? '' : 'mt-8 pt-8 border-t border-white/5'} w-full space-y-4`}>
                                    ...
                                    <div className="space-y-2 mt-6">
                                        ...
                                        <textarea
                                            ...
                                            placeholder="Ex: Mulher gr├ívida 30+ usando o chinelo ÔÇö alivia dor e incha├ºo nos p├®s e pernas. V├¡deo para Instagram Reels voltado para gestantes."
                                        ...
                                        />
                                        <p className="text-[9px] text-zinc-600">P├║blico-alvo, benef├¡cios, plataforma, tom do v├¡deo... A IA vai usar isso pra gerar mockups e prompts mais precisos.</p>
                                    </div>
                        ...
                        <motion.div key="s2" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                            ...
                            <div className="lg:col-span-4 space-y-6">
                                <div className="bg-white/[0.02] border border-white/[0.05] backdrop-blur-3xl rounded-3xl p-8 space-y-6 shadow-2xl">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[9px] font-semibold text-zinc-500 uppercase tracking-[0.2em]">Descri├º├úo AI (edit├ível)</label>
                                        <textarea
                                            value={editableDescription}
                                            onChange={(e) => setEditableDescription(e.target.value)}
                                        ...
                                        />
                                        <p className="text-[9px] text-zinc-600">Corrija detalhes do produto aqui se necess├írio</p>
                                    </div>
                                    ...
                                    <div className="pt-6 border-t border-white/5 space-y-4">
                                        ...
                                        <div className="flex flex-col gap-3">
                                            ...
                                            <button onClick={() => setOptions({ ...options, mode: 'scenery' })} className={`flex-1 py-4 text-[10px] font-semibold uppercase tracking-[0.1em] rounded-xl border transition-all duration-300 flex items-center justify-center gap-2 ${options.mode === 'scenery' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.15)]' : 'bg-white/[0.02] border-white/5 text-zinc-500 hover:bg-white/5'}`}>
                                                <Camera className="w-3.5 h-3.5" /> Cen├írios
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white/[0.02] border border-white/[0.05] backdrop-blur-3xl rounded-3xl p-8 shadow-2xl">
                                    ...
                                    {options.mode === 'script' ? (
                                        <div className="space-y-4">
                                            <textarea
                                                ...
                                                placeholder="Cole seu roteiro completo aqui. A IA vai quebr├í-lo em tantas cenas quantas ele precisar..."
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between">
                                                <label className="text-[9px] font-semibold text-zinc-500 uppercase tracking-[0.2em]">Cen├írios Sugeridos</label>
                                                <button onClick={regenerateSuggestions} className="w-7 h-7 flex items-center justify-center bg-white/5 hover:bg-cyan-500/20 text-zinc-500 hover:text-cyan-400 rounded-full transition-all" title="Gerar novas sugest├Áes">
                                    ...
                                    {options.mode === 'scenery' && (
                                        <div className="space-y-6">
                                            <p className="text-xs text-zinc-500">Configure as cenas baseadas nas suas fotos de local. A IA vai gerar prompts otimizados para v├¡deo ambientais e cinematogr├íficos.</p>

                                            {/* Camera Angle */}
                                            <div className="space-y-3">
                                                <label className="text-[9px] font-semibold text-zinc-500 uppercase tracking-[0.2em]">T├®cnica de C├ómera</label>
                                                <div className="grid grid-cols-2 gap-3">
                                            ...
                                            {/* Scene Action */}
                                            <div className="space-y-3">
                                                <label className="text-[9px] font-semibold text-zinc-500 uppercase tracking-[0.2em]">A├º├úo da Cena</label>
                                                <input
                                                    value={options.sceneAction}
                                                    onChange={(e) => setOptions({ ...options, sceneAction: e.target.value })}
                                                    placeholder="Ex: Casal caminhando ao p├┤r do sol, crian├ºas brincando na areia..."
                                            ...
                                            {/* Audio Design */}
                                            <div className="space-y-3">
                                                <label className="text-[9px] font-semibold text-zinc-500 uppercase tracking-[0.2em]">Design de ├üudio</label>
                                                <input
                                                    value={options.audioDesign}
                                                    onChange={(e) => setOptions({ ...options, audioDesign: e.target.value })}
                                                    placeholder="Ex: Ondas do mar + viol├úo ac├║stico suave"
                                                    className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-xs text-zinc-300 outline-none focus:border-emerald-500/50 transition-colors"
                                                />
                                            ...
                                            {/* Animation Speed */}
                                            <div className="space-y-3">
                                                <label className="text-[9px] font-semibold text-zinc-500 uppercase tracking-[0.2em]">Velocidade da Anima├º├úo</label>
                                        ...
                                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-10 pt-10 border-t border-white/5 space-y-6">
                                            ...
                                            <div className="mt-6 space-y-2">
                                                ...
                                                <input
                                                    ...
                                                    placeholder="Ex: @Alex @Maya ÔÇö mant├®m consist├¬ncia do personagem"
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-10 pt-10 border-t border-white/5">
                                        <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-5 space-y-4">
                                            <label className="text-[9px] font-semibold text-cyan-400 uppercase tracking-[0.2em] flex items-center gap-2">
                                                <Star className="w-3 h-3" /> DNA Estrat├®gico do Produto
                                            </label>
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex items-center gap-2 mt-4 pt-4 border-t border-white/5">
                                                    ...
                                                    <span className="text-[9px] text-zinc-500 ml-2 font-mono uppercase">Paleta Extra├¡da</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        >
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div>
                                                    <span className="text-xs font-semibold text-white block mb-1">Modo Econ├┤mico (Recomendado)</span>
                                                    <span className="text-[10px] text-zinc-400 leading-normal block">Gera apenas o primeiro mockup automaticamente. Voc├¬ economiza 66% dos cr├®ditos iniciais e renderiza os outros apenas se gostar dos prompts.</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <motion.div key="s3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                            <div className="flex items-center justify-between mb-8">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button onClick={goBackToConfigure} className="h-10 px-4 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.15em] text-cyan-400 hover:text-cyan-300 bg-cyan-500/10 hover:bg-cyan-500/20 rounded-full transition-all" title="Voltar para configura├º├úo sem perder mockups">
                                        ...
                                        <div className="flex-1 p-8 lg:p-10 flex flex-col">
                                            ...
                                            <div className="mt-8 pt-6 border-t border-white/5 flex flex-wrap items-center gap-6">
                                                ...
                                                <div className="ml-auto flex items-center gap-3">
                                                    <button
                                                        ...
                                                        title="M├ígica: Transformar rascunho em Blueprint Pro + Mockup"
                                                    >
                                                        <Wand2 className={`w-3.5 h-3.5 ${loadingIndices.includes(i) ? 'animate-pulse' : ''}`} />
                                                        <span className="text-[10px] font-bold uppercase tracking-wider">M├ígica</span>
                                                    </button>
                                                    <button
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={loadingIndices.includes(i)}
                                                        className="w-9 h-9 flex items-center justify-center bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white rounded-full transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                                                        title="Aleat├│rio (Novo Prompt + Mockup)"
                                                    >
                                            ...
                                            <div className="mt-4 bg-black/40 border border-white/5 rounded-2xl p-4 focus-within:border-amber-500/30 transition-colors">
                                                <textarea
                                                    ...
                                                    placeholder="O que voc├¬ n├úo gostou? Ex: 'A logo tem que ser menor', 'Muda a cor de fundo'..."
