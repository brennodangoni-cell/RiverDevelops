...
// =======================================================================
const BRAIN_MODELS = ["gemini-3.1-pro-preview", "gemini-2.0-flash", "gemini-1.5-pro"];
const ANALYSIS_MODELS = ["gemini-3.1-pro-preview", "gemini-2.0-flash", "gemini-1.5-pro"];
const IMAGE_MODELS = ["gemini-3.1-flash-image-preview", "gemini-3.1-pro-preview", "gemini-1.5-pro"];
...
function classifyError(e: any): AIError {
    ...
    if (msg.includes('429') || msg.includes('RESOURCE_EXHAUSTED') || msg.includes('rate')) {
        return new AIError('Limite de requisi├º├Áes atingido. Aguarde alguns segundos.', 'RATE_LIMIT', true);
    }
    if (msg.includes('404') || msg.includes('not found') || msg.includes('NOT_FOUND')) {
        return new AIError('Modelo n├úo dispon├¡vel. Tentando modelo alternativo...', 'MODEL_NOT_FOUND', true);
    }
    if (msg.includes('timeout') || msg.includes('DEADLINE')) {
        return new AIError('Timeout na requisi├º├úo. Tentando novamente...', 'TIMEOUT', true);
    }
...
function parseBase64(base64: string): { data: string; mimeType: string } {
    ...
    if (match) {
        ...
        // Force image mime type ÔÇö octet-stream means the browser didn't recognize it
        if (!mimeType.startsWith('image/')) mimeType = 'image/jpeg';
        return { data: match[2], mimeType };
    }
    // No prefix ÔÇö assume raw base64 JPEG
...
export async function analyzeProduct(imagesBase64: string[], marketingContext?: string): Promise<ProductAnalysis> {
    ...
    if (!apiKey) throw new AIError("Chave API do Gemini n├úo configurada.", "API_KEY_MISSING");
        ...
        config: {
            ...
            responseSchema: {
                ...
                properties: {
                    ...
                    productType: { type: Type.STRING, description: "A short category name in Portuguese (e.g., 'T├¬nis', 'Garrafa de Skincare', 'Eletr├┤nico')." },
...
RETURN a JSON:
1. "description" (ENGLISH): Ultra-detailed description of the location ÔÇö architecture, nature, lighting conditions, colors, textures, atmosphere, time of day, weather.
2. "locationType" (PORTUGUESE): Short category ÔÇö e.g., "Praia", "Floresta", "Cidade Urbana", "Interior de Casa"
3. "mood" (ENGLISH): The emotional mood this location evokes ÔÇö e.g., "serene and peaceful", "gritty and urban"
...
export async function generatePrompts(
    ...
    Scene 1 ÔÇö Reveal: Highlighting one version with a dramatic orbit shot.
    Scene 2 ÔÇö Interaction: Another variant being used/worn in close-up.
    Scene 3 ÔÇö Details: Macro shots of multiple variants in a premium layout.
    ` : `
    GOAL: Create 3 cinematic video scenes (10 seconds each) for a high-end commercial:
    Scene 1 ÔÇö THE HOOK: Wide establishing shot. Reveal the product in a majestic environment.
    Scene 2 ÔÇö THE STORY: Medium tracking shot. Show the core benefit and product DNA in action.
    Scene 3 ÔÇö THE DETAILS: Extreme macro. Focus on the sharpest textures and perfectly legible branding.
    `)}
    `;
    ...
    Scene A ÔÇö Reaction or alternative angle.
    Scene B ÔÇö Dynamic stylized transition.
    Scene C ÔÇö Final majestic outro with a focus on brand identity.
        `;
    }
...
// =======================================================================
// 3. GENERATE MOCKUP (Fix #1 ÔÇö Now receives original product images!)
// =======================================================================
